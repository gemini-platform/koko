...
func (s *JMService) CreateAssetGateway(name, ip string, port int, domain string) (gateway model.Gateway, err error) {
...
func (s *JMService) UpdateAssetGateway(id, name, ip string, port int, domain string) (gateway model.Gateway, err error) {
	params := map[string]interface{}{
		"name":     name,
		"ip":       ip,
		"port":     port,
